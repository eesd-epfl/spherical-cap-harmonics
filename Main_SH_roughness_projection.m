...
% This file is for the roughness projection on a spherical harmonics (closed objects).
%% Paths
% function Main_SH_roughness_projection
close all;
clear; clc; tic
addpath('code')
addpath('stlTools')
...
analysis_mat_file = 'rec_l_10_SH_rock_mesh_manifold.mat';
...
% filter_query = strcat('selected_verts = find(v(v(:,3)>0));'); % for the hemisphere mesh
% filter_query = strcat('selected_verts = [];'); % for the no selection
eval(filter_query);

if isempty(selected_verts)
    selected_verts = 1:length(v);
end
...
% Solve for \phi and \theta from the donor mesh patch on a sphere
map_ = spherical_conformal_map(v,f);
map = mobius_area_correction_spherical(v,f,map_);
stlWrite(['rec_output/para_output_SH_', surface_name], f, map,'mode','ascii')

if plot_figures
    paraview_patch(map, f)
    title('Input donor mesh parametrisation on a unit sphere')
end
...
reconstruction = real(SH_basis_range(phis, thetas, k_range, qm_k));
